...
const Card = () => {
    ...
    useEffect(() => {
        let funcionCarga = async () => {
            let { respuestaJson, response } = await actions.useFetch("/apis/fake/contact/agenda/agenda_de_marce")
                ...
                <ul className="list-group list-group-flush bg-dark">
                    {lista && lista.length > 0 ? <>
                        {lista.map((item, index) => {
                            return (
                                <li key={index} className="list-group-item d-flex mt-2 mb-2 p-2 bg-dark border-white">
                                    ...
                                    <div>
                                        <Link to={"/edit-contact/" + item.id}><i className="icon fa-solid fa-pencil bg-white p-2 rounded"></i></Link>
                                        <i className="icon fa-solid fa-trash bg-white p-2 rounded ms-3" onClick={async () => {
